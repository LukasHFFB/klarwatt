/**
 * Solaranlage (Hausdach) Ertragsrechner — Configuration
 */
...
/**
 * Simultaneous-use factor for rooftop systems (typically lower than BKW due to size).
 * Without battery: ~20-35%.
 */
export const getSimultaneousFactor = (jahresverbrauch: number, jahresertrag: number): number => {
    ...
    // If we produce way more than we consume, the factor drops.
    const ratio = jahresverbrauch / jahresertrag;
    return Math.min(0.70, Math.max(0.20, 0.15 + (ratio * 0.25)));
};

export const SolaranlageConfig: CalculatorConfig = {
    slug: 'ertrag',
    category: 'hausdach',
    breadcrumbLabel: 'Hausdach',
    title: 'Solaranlage Ertragsrechner',
    description: 'Berechnen Sie Ertrag, Eigenverbrauch und Wirtschaftlichkeit Ihrer Dachanlage inkl. Einspeisevergütung.',
    showIrradianceChart: true,

    inputs: [
        PLZ_INPUT,
        { ...KWP_LARGE_INPUT, label: 'Anlagenleistung (PV)', default: 10 },
        AZIMUTH_INPUT,
        TILT_INPUT,
        JAHRESVERBRAUCH_INPUT,
        STROMPREIS_INPUT,
        STROM_STEP_INPUT,
        { ...PURCHASE_PRICE_INPUT, default: 15000, label: 'Investitionskosten (Netto)' },
    ],
    ...
    calculate: (inputs, irradiance = 1000) => {
        ...
        const amortisation = amortisationYears; // Use the more precise multi-year amortisation
